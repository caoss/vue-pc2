<template>
  <div id="home">
    ...
    <ul>
      <li>
        注册总人数：
        <span class="red">{{ totalRegister }}</span>
      </li>
      <li>
        登录激活总人数：
        <span class="red">{{ totalActiver }}</span>
      </li>
      <li>
        最高月活人数：
        <span class="red">{{ topMouthActiver }}</span>
      </li>
      <li>
        今日登录人数：
        <span class="red">{{ todayLogin }}</span>
      </li>
    </ul>
  </div>
</template>
...
export default {
  ...
  asyncData({ store, route }) {
    ...
    // return store.dispatch("list/getList",{pageNo:1,pagwSize:10});
  },
  name: "home",
  title: "Home",
  data() {
